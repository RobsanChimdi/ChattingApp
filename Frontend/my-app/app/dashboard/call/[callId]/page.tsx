...
export default function CallPage() {
  ...
  const { createPeerConnection, closeAllConnections, isNegotiating } = useWebRTC(callId);
  ...
  useEffect(() => {
    if (isAuthenticated && callId && !activeCall && !isLoading && !isJoining) {
      joinCall(callId).catch(console.error);
    }
  }, [isAuthenticated, callId, activeCall, isLoading, isJoining, joinCall]);
    ...
    <div className="flex h-screen flex-col bg-black text-white">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 relative">
          ...
          {/* Audio indicators overlay */}
          <div className="absolute top-4 left-4 flex space-x-2">
            {participants.map((participant) => (
              <AudioIndicator
                key={participant.user.id}
                stream={remoteStreams.get(participant.user.id.toString()) || null}
          ...
          {/* Participants panel toggle for mobile */}
          {isMobile && (
            <Button
              variant="outline"
              size="sm"
        ...
        {showParticipants && (
          <div className={cn(
            "w-80 border-l border-white/10 bg-black/50 backdrop-blur-sm overflow-y-auto",
            isMobile && "absolute right-0 top-0 bottom-0 z-10"
          )}>
            <div className="p-4">
                  ...
                  >
                    ...
                    {isInitiator && participant.user.id !== user?.id && (
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          if (confirm(`Mute ${participant.user.username}?`)) {
                            muteParticipant(participant.user.id);
                          }
                        }}
                      >
                        Mute
                ...
                <div className="mt-6 p-3 bg-white/5 rounded-lg">
                  ...
                  <div className="space-y-1 text-xs text-white/60">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>{callStats.videoBitrate} kbps</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Audio Bitrate</span>
                      <span>{callStats.audioBitrate} kbps</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Packet Loss</span>
                      <span>{(callStats.packetLoss * 100).toFixed(1)}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Jitter</span>
                      <span>{callStats.jitter} ms</span>
                    </div>
                    <div className="flex justify-between">
                      <span>RTT</span>
                      <span>{callStats.roundTripTime} ms</span>
                    </div>
                  </div>
                </div>
              )}
